...
_FACT_PATTERNS = [
    ...
    re.compile(r"\bi('m| am) ([\w ]+)", re.IGNORECASE),
...
def _extract_facts(user_input: str) -> list[str]:
...
def _append_to_memory(facts: list[str], memory_dir: Path) -> None:
    ...
    if new_entries:
        with memory_file.open("a", encoding="utf-8") as fh:
            fh.write("\n".join(new_entries) + "\n")
        logger.debug("Wrote %d new facts to MEMORY.md", len(new_entries))
...
async def pre_response_hook(user_input: str, **kwargs: Any) -> None:
    ...
    Future: pre-fetch memory context for injecting into system prompt.
    """
